...
int main (int argc, char *argv[])
{
    ...
    MPI_Comm_split(MPI_COMM_WORLD, old_rank / GROUP_SIZE, old_rank, &custom_group);
    ...
    MPI_Send(&new_rank, 1, MPI_INT, (new_rank + 1) % new_size, 0, custom_group);
    printf("Process [%d] from group [%d] sent [%d].\n", new_rank,
            old_rank / GROUP_SIZE, new_rank);

    // Receive the rank.
    MPI_Recv(&recv_rank, 1, MPI_INT, (new_rank - 1 + new_size) % new_size, 0, custom_group, MPI_STATUS_IGNORE);
